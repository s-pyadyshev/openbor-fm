...
void grabcheckG2(void Ani1, void Ani2, void Ani3)
{ // Hero's groundgrab checker
// This script prevents hero from grabbing airborne enemies, bikers, jetpacks, special enemies and non-enemy entities
//  Ani1 = Grab animation to play
//  Ani2 = Animation to play if hit enemy are bikers or jetpacks
...
// Otherwise it returns 1.
    ...
    if(getentityproperty(target, "type") == openborconstant("TYPE_PLAYER")){
      ...
      if(Tname == "bolo_" || Tname == "bolo2" || Tname == "abobo" || Tname == "mibobo" || Tname == "mibobo2" || Tname == "chin" || Tname == "chin6" )
      { // Normal Bikers
        return 0;
      } else {
        return 1;
